...
class CLoss(nn.Module):
    ...
    def structure_modality_contrastive(self, str_embdding, vis_embdding):
        """
        :param str_embdding: [num_ent, emb_dim]
        :param vis_embdding: [num_ent, emb_dim]
        :return:
        """
        ...
        neg_vis_feat = vis_embdding[neg_sample_id]  # [num_ent, num_sample, emb_dim]
        str_samples = torch.cat([str_embdding.unsqueeze(1), neg_str_feat], 1)  # [num_ent, 1+num_sample, emb_dim]
        vis_samples = torch.cat([vis_embdding.unsqueeze(1), neg_vis_feat], 1)  # [num_ent, 1+num_sample, emb_dim]
        s2v_score = torch.matmul(vis_samples, str_embdding.unsqueeze(2)).squeeze(
            2) / self.svc_temperature  # [num_ent, num_sample+1]
        v2s_score = torch.matmul(str_samples, vis_embdding.unsqueeze(2)).squeeze(
            2) / self.svc_temperature  # [num_ent, num_sample+1]
...
class TuckER_Decoder(nn.Module):
    ...
    def forward(self, ctx_emb, rel_emb, emb_ent):
        ...
        r = self.mm(rel_emb, self.W.view(rel_emb.size(1), -1))  # [batch_size, emb_dim * emb_dim]
